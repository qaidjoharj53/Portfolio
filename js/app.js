...
var flagshipProjects = [
	...
	},
	{
		...
		subtitle:
			"OCR Document Processing and Data Extraction from Government IDs",
...
var communityWork = [
	...
	{
		title: "Udaipur Toloba App",
		client: "Udaipur Toloba",
		date: "Live on App Store and Google Play",
		problem:
		...
		stack: ["React Native", "Go", "Expo", "Supabase", "GCP"],
		links: [
			{
				label: "App Store",
				url: "https://apps.apple.com/us/app/udaipur-toloba/id6789900086",
			},
			{
				label: "Google Play",
				url: "https://play.google.com/store/apps/details?id=com.udaipurtoloba.udaipurtolobaapp",
			},
		],
	},
];
...
if (experienceContainer) {
	experienceData.forEach(function (item, index) {
		...
		if (item.architecture) {
			...
			arch.innerHTML =
				"<strong>Architecture:</strong> " + item.architecture;
...
if (communityContainer) {
	communityWork.forEach(function (item, index) {
		...
		var linksDiv = createEl("div", "project-links");
		var itemLinks =
			item.links || (item.url ? [{ label: "Visit", url: item.url }] : []);
		itemLinks.forEach(function (itemLink) {
			var link = createEl("a", "project-link", itemLink.label + " ->");
			link.href = itemLink.url;
			link.target = "_blank";
			link.rel = "noopener noreferrer";
			linksDiv.appendChild(link);
		});
		if (itemLinks.length) article.appendChild(linksDiv);
